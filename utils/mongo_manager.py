...
class MongoManager():
    ...
    def get_mention_count_by_seeds(self,experiment_id,seed_ids,ontology):
        collection = "entity"
        query = ([{"$match":{"confidence":{"$gt":0.6},"id_experiment":experiment_id,"seed":{"$in":seed_ids},"types":{"$not":{"$size":0}}}},{"$project":{"seed":1,"types":1}},{"$unwind":"$types"},{"$group":{"_id":"$types","count":{"$sum":1}}},{"$sort":{"count":-1}}])
        return list(self.db[collection].aggregate(query))[:10]
